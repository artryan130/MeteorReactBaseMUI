...
interface ITasksList extends IDefaultListProps {
    remove: (doc: ITasks) => void;
    taskss: ITasks[];
...
const TasksList = (props: ITasksList) => {
    ...
    const idTasks = shortid.generate();
        ...
        <PageLayout title={'Lista de Exemplos'} actions={[]}>
            ...
            <RenderComPermissao recursos={[Recurso.EXEMPLO_CREATE]}>
                ...
                >
                    <Fab
                        id={'add'}
                        onClick={() => navigate(`/tasks/create/${idTasks}`)}
                        color={'primary'}
                    >
                        <Add />
                    </Fab>
...
let onSearchTasksTyping: any;

export const TasksListContainer = withTracker((props: IDefaultContainerProps) => {
    ...
    const subHandle = tasksApi.subscribe('tasksList', filter, {
        sort,
        limit,
        skip,
    });
    const taskss = subHandle?.ready() ? tasksApi.find(filter, { sort }).fetch() : [];

    return {
        taskss,
        loading: !!subHandle && !subHandle.ready(),
        remove: (doc: ITasks) => {
            tasksApi.remove(doc, (e: IMeteorError) => {
                if (!e) {
                    showNotification &&
                        showNotification({
                            type: 'success',
        ...
        onSearch: (...params: any) => {
            onSearchTasksTyping && clearTimeout(onSearchTasksTyping);
            onSearchTasksTyping = setTimeout(() => {
                config.pageProperties.currentPage = 1;
                subscribeConfig.set(config);
                tasksSearch.onSearch(...params);
            }, 1000);
        },
        total: subHandle ? subHandle.total : taskss.length,
